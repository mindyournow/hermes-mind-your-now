...
Tools that return lists (`list_tasks`, `list_events`, `list_habits`, `list_projects`, `recall`, etc.) apply client-side limits and mark truncated results so the model knows it's looking at a slice. Task and habit reads stop after enough matching rows and never scan more than 50 API pages or 10,000 source tasks; `_collectionComplete: false` identifies a result stopped early or by that ceiling. `myn_ynab.list_transactions` requires `sinceDate` (or the `date` alias) so the upstream financial-data read is bounded.

Bulk list actions and the `schedule_all`/`reschedule` planning actions support `dryRun`; planning dry-runs use the side-effect-free `/api/v1/customers/planning-context` endpoint for customer identity and timezone, exclude assigned tasks that live planning does not own, and return up to `previewLimit` slimmed tasks (default 50, maximum 200) without invoking a scheduling endpoint. A complete scan reports the full candidate count; a safety-capped scan sets `collectionComplete: false` and `countIsLowerBound: true`. The engine's resulting dates and placements remain unavailable. These markers and capability boundaries prevent the model from mistaking a bounded candidate preview for complete engine decisions.
